<?php
	...
	$PublicIP = "174.126.235.27";
	use xPaw\SourceQuery\SourceQuery;
	//LOOP1 - KF2
	// Edit this ->

	
	define( 'KFADDR', $PublicIP );
	define( 'KFPORT', 27015 );
	define( 'KFTIMEOUT',     1 );
	...
	define( 'PZCasualADDR', $PublicIP );
	define( 'PZCasualPORT', 16053 );
	define( 'PZCasualTIMEOUT',     1 );
	define( 'PZCasualENGINE',      SourceQuery::SOURCE );
	// Edit this <-
	//LOOP2 - PZServer
	$PZCasualQuery = new SourceQuery( );

	$PZCasualInfo    = [];
	$PZCasualRules   = [];
	$PZCasualPlayers = [];
	$PZCasualException = null;

	try
	{
		$PZCasualQuery->Connect( PZCasualADDR, PZCasualPORT, PZCasualTIMEOUT, PZCasualENGINE );
		//$Query->SetUseOldGetChallengeMethod( true ); // Use this when players/rules retrieval fails on games like Starbound
		
		$PZCasualInfo    = $PZCasualQuery->GetInfo( );
		$PZCasualPlayers = $PZCasualQuery->GetPlayers( );
		$PZCasualRules   = $PZCasualQuery->GetRules( );
	}
	catch( Exception $PZCasuale )
	{
		$PZCasualException = $PZCasuale;
	}
	finally
	{
		$PZCasualQuery->Disconnect( );
	}

	define( 'PZHardcoreADDR', $PublicIP );
	define( 'PZHardcorePORT', 16034 );
	define( 'PZHardcoreTIMEOUT',     1 );
	define( 'PZHardcoreENGINE',      SourceQuery::SOURCE );
	// Edit this <-
	//LOOP2 - PZServer
	$PZHardcoreQuery = new SourceQuery( );

	$PZHardcoreInfo    = [];
	$PZHardcoreRules   = [];
	$PZHardcorePlayers = [];
	$PZHardcoreException = null;

	try
	{
		$PZHardcoreQuery->Connect( PZHardcoreADDR, PZHardcorePORT, PZHardcoreTIMEOUT, PZHardcoreENGINE );
		//$Query->SetUseOldGetChallengeMethod( true ); // Use this when players/rules retrieval fails on games like Starbound
		
		$PZHardcoreInfo    = $PZHardcoreQuery->GetInfo( );
		$PZHardcorePlayers = $PZHardcoreQuery->GetPlayers( );
		$PZHardcoreRules   = $PZHardcoreQuery->GetRules( );
	}
	catch( Exception $PZHardcoree )
	{
		$PZHardcoreException = $PZHardcoree;
	}
	finally
	{
		$PZHardcoreQuery->Disconnect( );
	}
